...
import { getPopularMovies } from '../functions/getMovies'
...
const PopularMovies = () => {
  ...
  useEffect(() => {
    const gPM = async () => {
      ...
      setPopularMovies(popularMoviesResponse.results.slice(0, 3))
    }
    ...
    <div className="mt-10">
      ...
      <ul className="mt-4 text-gray-400 text-xs space-y-3">
        {popularMovies.map((movie) => (
          <li className="flex space-y-3 space-x-3 " key={movie.id}>
            <img
              src={`${process.env.NEXT_PUBLIC_TMDB_IMAGE_URL}/w220_and_h330_face${movie.poster_path}`}
